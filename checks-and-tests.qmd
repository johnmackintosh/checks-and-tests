...
partial matching - you can be lazy and only type the first few letters. This is OK for this very simple example, but not for real-life code
...
See [phsmethods](https://github.com/Public-Health-Scotland/phsmethods/blob/master/R/chi_check.R)  
...
We can deal with the first three quite quickly with the [{checkmate}](https://mllg.github.io/checkmate/articles/checkmate.html) package
...
## further tests - am I in the best band in the world?

```{r}
is_metallic <- function(m){

checkmate::check_character(m) # are we passing a character ?
  
checkmate::assert_choice(m, 
                         choices = c("Lars", "James", "Kirk", 
                                      "Rob", "Jason", "Cliff"),
                         .var.name = "m") # is m in this set of values?

  if (m %in% c("Jason", "Cliff")) {
    out <- paste0(m, " _was_ Metallic")
    return(out)
  }                      
...
![I know how he feels](sad_ben.jpg){fig-align="left"}
